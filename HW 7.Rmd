...
ym_lm5 <- lm(value ~ harmonic(BRW_flask, m = 1), data = BRW_flask) #single harmonic pair (m = 1)
ym_lm6 <- lm(value ~ harmonic(BRW_flask, m = 5), data = BRW_flask) #5th order harmonic (m = 5)
...
ym_gam <- gam(value ~ s(time(BRW_flask), k=43, bs="ts")
                    + s(time(cycle(BRW_flask)), k=12, bs="cc"), data=BRW_flask)
plot(ym_gam)
```

...
The AIC for the GAM model is `r floor(AIC(ym_gam))`, which is lower than that for the quadratic model with a fifth-order harmonic by `r floor(AIC(ym_gam) - AIC(ym_lm12))`.  It also uses fewer degrees of freedom, with an edf of `r round(summary(ym_gam)$s.table[2,1], 2)` versus the 14 used by the quadratic with fifth-order harmonic model.  
...
# it's a start but doesn't work yet
...
ar1_function <- function(x){
  p_value <- summary(gls(x ~ Year, correlation = corARMA(.5,p=1,q=0)))$tTable[8]
  decision <- ifelse(p_value < 0.05, 1, 0)
  return(decision)
}

ma1_function <- function(x){
  p_value <- summary(gls(x ~ Year, correlation = corARMA(.5, p=0,q=1)))$tTable[8]
  decision <- ifelse(p_value < 0.05, 1, 0)
  return(decision)
}
...
clusterExport(clust, c("Data1", "shuffle", "gls", "corARMA", "Year"))

t_ar_type_1 <- parApply(clust, ar_sim_data, 2, ar1_function)

t_ma_type_1 <- parApply(clust, ar_sim_data, 2, ma1_function)

# Since the seed is set for the cores this time, this won't change between runs

...
ar1_function <- function(x){
  p_value <- summary(gls(x ~ Year, data=Data1, correlation = corARMA(.5,p=1,q=0)))$tTable[8]
...
ma1_function <- function(x){
  ...
  print(formula)
  p_value <- summary(gls(x ~ Year, data=Data1, correlation = corARMA(.5, p=0,q=1)))$tTable[8]
  decision <- ifelse(p_value < 0.05, 1, 0)
  return(decision)
}
...
clusterExport(clust, c("Data1", "shuffle", "gls", "corARMA", "p_value"))

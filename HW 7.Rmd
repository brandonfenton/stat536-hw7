...
The AIC for the GAM model is `r floor(AIC(ym_gam))`, which is lower than that for the quadratic model with a fifth-order harmonic by `r floor(AIC(ym_lm12) - AIC(ym_gam))`.  
...
ar_sim_data <- replicate(250, arima.sim(n = 109, model = list(ar = c(0.6)),
>>>>>>> 452712b69c49c64667ed2e0d234bd9f37a3111a2
...
ma1_function <- function(x){
  z <- data.frame(Year = Data1$Year, x)
  #formula <- as.formula("x ~ Year")
  #print(formula)
  p_value <- summary(gls(x ~ Year, data = z, 
                         correlation = corARMA(p = 1, q = 1)))$tTable[8]
  decision <- ifelse(p_value < 0.05, 1, 0)
  return(decision)
}
